...
- **Tagged releases sign and publish again.** Both jobs in the release workflow set `egress-policy: block` with `fulcio`, `rekor` and `tuf-repo-cdn` allowed but not `timestamp.sigstore.dev`, so cosign reached the CA and the transparency log and then died countersigning, on `Post "https://timestamp.sigstore.dev/api/v1/timestamp": connection refused`. The endpoint became mandatory when `sigstore/cosign-installer` moved from v3 to v4: cosign v3 signs into the new bundle format, which always countersigns with the sigstore RFC 3161 timestamp authority, where cosign v2 never called it. Both jobs push their artifact before signing, so the first v0.6.1 tag run left unsigned image and chart layers in GHCR and never reached the step that creates the GitHub release.
...
- **Added CodeQL (SAST), `dependency-review`, and OpenSSF Scorecard workflows**, plus a `pre-commit` config (`gitleaks` secret scanning, `golangci-lint`, `shellcheck`, and end-of-file/trailing-whitespace fixers).
